...
def self_supervised_model(input_tensor, isTraining, drop_rate, hidden_nodes = 128, stride_mp = 4):
    ...
    ## conv block 1
    conv1     = main_branch 
    conv1     = tf.layers.max_pooling1d(conv1, pool_size = conv1.get_shape()[1], strides=stride_mp, padding='valid', name = 'GAP1')    
    ...
    ## conv block 2
    conv2     = main_branch 
    conv2     = tf.layers.max_pooling1d(conv2, pool_size = conv2.get_shape()[1], strides=stride_mp, padding='valid', name = 'GAP2') 
    ...
    ## conv block 3
    conv3     = main_branch 
    conv3     = tf.layers.max_pooling1d(conv3, pool_size = conv3.get_shape()[1], strides=stride_mp, padding='valid', name = 'GAP3') 
...
def supervised_model(input_dimension = 128, output_dimension = 3, dropout = 0.4, hidden_nodes = 512, lr_super = 0.001, L2 = 0):
...
def supervised_model_seed(x_tr_feature, 
                                  y_tr, 
                                  x_te_feature, 
                                  y_te, 
                                  identifier, 
    ...
    log = model.fit(x_tr_feature, y_tr, epochs=epoch_super, batch_size=batch_super, verbose=0, callbacks=[tb], validation_data = (x_te_feature, y_te), shuffle = True) 
    print('Loss:', log.history['loss'][0])
    ...
    print('For Training')
    tr_accuracy, tr_precision, tr_recall, tr_f1_score = utils.model_result_store(y_tr, y_tr_pred, os.path.join(result, str("tr_" + identifier + ".csv")), kfold)

    print('For Testing')
    te_accuracy, te_precision, te_recall, te_f1_score = utils.model_result_store(y_te, y_te_pred, os.path.join(result, str("tr_" + identifier + ".csv")), kfold)

    tr_score = [tr_accuracy, tr_precision, tr_recall, tr_f1_score]
    te_score = [te_accuracy, te_precision, te_recall, te_f1_score]

    return model, tr_score, te_score